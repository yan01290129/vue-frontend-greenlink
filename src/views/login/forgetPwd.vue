<template>
  <el-row>
    <el-col :span="24" class="forget-pwd-wrap">
        ...
        <div class="content-wrap step-two" v-show="active==1">
          <el-form ref="setPwdForm" :rules="pwdRuleValidate" :model="setPwdForm" label-width="11rem">
            <el-form-item label="新密码" required prop="password">
              <el-input type="password" v-model="setPwdForm.password" placeholder="请输入6-12位数新密码" size="large"
                         @keyup.enter.native="setPwd()"></el-input>
            </el-form-item>
            <el-form-item label="验证密码" required prop="checkPassword">
              <el-input type="password" placeholder="请确认新密码" v-model="setPwdForm.checkPassword" size="large"
                         @keyup.enter.native="setPwd()"></el-input>
            </el-form-item>
            <el-form-item class="submit-box">
  ...
  export default {
    data() {
      let validatePhone = (rule, value, callback) => {
        ...
        apiEmployeePhoneExist({phone: value}).then(res => {
          if (res.data) {//系统中存在
    ...
    methods: {
      ...
      getCaptcha() {
        this.time = 60;
        this.timer();
        forgetPwdGetCaptcha({phone: this.verifyPhoneForm.phone}).then((res) => {
          if (res.data.response) this.verifyPhoneForm.captcha = res.data.response;
        }).catch((err) => {
          this.$message.error(err.message);
        })
      },
      verifyPhone() {
        validForgetCaptcha(this.verifyPhoneForm).then((res) => {
      ...
      /**step two */
      setPwd() {
        forgetPwd(this.setPwdForm).then((res) => {
          this.active = 2;
          let _this = this;
          setTimeout(function () {
...
.forget-pwd-wrap .footer {
    text-align: center;
    font-size: .6rem;
    color: #8492a6;
}
.forget-pwd-wrap .footer a {
    color: #4eb940;
    font-size: .7rem;
}
</style>
